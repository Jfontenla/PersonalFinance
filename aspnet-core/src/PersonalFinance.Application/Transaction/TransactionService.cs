...
namespace PersonalFinance
{
    public class TransactionService : PersonalFinanceAppServiceBase, ITransactionService
    {
        ...
        public async Task<List<TransactionListDTO>> GetAll(GetAllTransactionsByAccountInput input)
        {
            var transactions = await _transactionRepository.GetAll()
                .Include(t => t.Accounts)
                    .ThenInclude( a => a.Account)
                    .ThenInclude( a => a.User)
                .Where(t => t.Accounts.Select(a => a.Account.UserId).Contains(input.AccountId))
                .ToListAsync();
